...
你作为一名专业内容审核员，需要评估生成的章节内容质量。请从以下几个方面进行评估：
1. **内容完整性** - 是否全面覆盖了章节主题的核心要点
2. **信息准确性** - 是否基于提供的信息来源，避免主观臆断
3. **结构逻辑性** - 是否有清晰的逻辑结构
4. **深度与专业性** - 是否达到专业研究报告的标准

如果内容质量合格，请回复"满足要求"。如果存在不足，请明确指出需要改进的具体方面和建议。
"""
...
async def process_section(section: Dict[str, str], research_topic: str, section_index: int,
                          max_retries: int = 3) -> str:
    """
    处理单个章节：检索 -> 抓取 -> 起草 -> 反思 -> 改进（循环）
    """
    section_title = section.get("section_title")
    search_keywords = section.get("search_keywords")

    print(f"\n--- 开始处理章节 {section_index + 1}: {section_title} ---")

    best_draft = ""
    improvement_suggestions = ""

    for retry in range(max_retries):
        print(f"  第 {retry + 1} 轮生成/改进...")

        # 1. 检索
        section_query = f"{section_title} {search_keywords}"
        if retry > 0 and improvement_suggestions:
            section_query = f"{section_query} {improvement_suggestions}"
        ...
        **原始网页内容:**
        ...
        {"**改进建议:** " + improvement_suggestions if improvement_suggestions else ""}
        ...
        **生成的章节内容:**
        ...
        try:
            ...
            reflection_text = reflection_result.final_output
        except Exception as e:
            reflection_text = f"反思评估失败: {e}"

        print(f"  反思结果: {reflection_text[:200]}...")

        # 检查是否满足要求
        if "满足要求" in reflection_text or retry == max_retries - 1:
            best_draft = current_draft
            print(f"  ✓ 章节 {section_index + 1} 完成")
            break
        else:
            improvement_suggestions = reflection_text
            print(f"  → 需要改进，继续下一轮...")
...
"""
    ...
    # 创建所有章节的处理任务
    section_tasks = [
        process_section(section, query, i)
