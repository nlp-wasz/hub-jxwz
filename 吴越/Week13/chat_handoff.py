...
async def chat(user_name:str, session_id: Optional[str], task: Optional[str], content: str, tools: List[str] = []):
    ...
    if session_id:
        with SessionLocal() as session:
            ...
            if not record:
                #（没有session_id，在ChatSessionTable中创建一条用户的历史聊天记录，在ChatMessageTable表中创建聊天记录对应的内容）
                init_chat_session(user_name, content, session_id, task)

    #将用户的内容存储到ChatMessageTable表中
    ...
    stocks_mcp_tools_filter: ToolFilterStatic = ToolFilterStatic(
        allowed_tool_names=["get_stock_codes", "get_index_code","get_industry_code","get_board_info","get_stock_rank","get_month_line","get_week_line","get_day_line","get_stock_info","get_stock_minute_data"])
    mcp_server_1=MCPServerSse(
        name="SSE Python Server",
        params={"url":"http://localhost://8900/sse"},
        cache_tools_list=False,
        tool_filter=stocks_mcp_tools_filter,
        client_session_timeout_seconds=20,
    ...
    session = AdvancedSQLiteSession(
        session_id=session_id, # 与 系统中的对话id 关联，存储在关系型数据库中，一次对话就是一个session
    ...
    chat_agent=Agent(
        name="Chat Assistant",
        instructions="""1. 保持对话的自然和流畅，以轻松愉快的语气回应用户。
                        2. 避免过于专业或生硬的术语，除非用户明确要求。
                        3. 倾听用户的表达，并在适当的时候提供支持、鼓励或趣味性的知识。
                        4. 确保回答简洁，富有情感色彩，不要表现得像一个没有感情的机器。
                        5. 关键词：友好、轻松、富有同理心。""",
    ...
    if not tools or len(tools) == 0:
        ...
        # 这一条大模型回答，存储到ChatMessageTable表中
    ...
    # 需要调用mcp 服务进行回答
        ...
        async with mcp_server:
            ...
            if set(need_viz_tools) & set(tools):#两个集合的交集
