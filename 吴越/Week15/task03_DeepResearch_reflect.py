...
#改进
reflecction_system_prompt="""
你作为一名专业的内容审核员，你需要结合用户的提问和生成的回答，判断回答是否满足了提问的要求。如果满足，则直接回答，不需要有其他输出。如果不满足，则输出还需要分析的子问题，每行一个子问题。
"""
reflecctionAgent=Agent(
"reflecction Drafting Specialist",
    instructions=drafting_system_prompt,
    model=OpenAIChatCompletionsModel(
        model="qwen-max",
        openai_client=external_client,
...
async def process_section(index:int,section:Dict[str:Any],query:str,max_reflections:int)->str:
    """处理单章节内容，加上reflect机制"""
    ...
    try:
        ...
        urls_to_crawl = [res['url'] for res in search_results if res.get('url')][:2]  # top2相关的网页URL
    except:
        print("Warning: Failed to parse search results for crawl.")
        urls_to_crawl = []


    crawled_tasks=[async_crawl_jina(url) for url in urls_to_crawl]
    crawled_content=await asyncio.gather(*crawled_tasks)

    raw_materials = "\n\n".join(f"---URL:{url}---\n{content[:3000]}\n"
                                for url,content in zip(urls_to_crawl,crawled_content))
    ...
    **章节结果摘要：**{section_search_results_str[:3000]}
    ...
    for attempt in range(max_reflections):
        ...
        reflection_prompt = f"""
            ...
            **章节主题:**{section_title}
            ...
            下面是撰写的章节详细内容，请判断是否满足章节主题。如果满足，只输出满足二字；如果不满足，请给出建议，每行一个。
            {section_draft.final_output}
        """

        reflection=await Runner.run(reflecctionAgent,reflection_prompt)
        reflection_text=reflection.final_output.strip()

        if reflection_text=="满足":
...
async def deep_research(query: str, max_sections: int = 5,max_reflections:int=2) -> str:
    """
...
"""
    try:
        ...
        outline_json = json.loads(outline_response.final_output.strip("```json").strip("```"))#解析jsson
    ...
    #并发处理多章节内容
    section_tasks=[asyncio.create_task(process_section(i,section,query,max_reflections)) for i, section in sections]
