...
def sentiment_classification(text: Annotated[str, "The text to analyze"]):
    """Classifies the sentiment of a given text."""
    try:
        response = requests.get(f"https://whyta.cn/api/nlp/sentiment?key={TOKEN}&text={text}")
        result = response.json()
        if result.get("code") == 200 and "sentiment" in result:
            return {"sentiment": result["sentiment"], "confidence": result.get("confidence", 0), "analysis": result.get("analysis", "")}
        return {"error": "Failed to analyze sentiment", "details": result}
