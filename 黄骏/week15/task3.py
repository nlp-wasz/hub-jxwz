...
async def run_drafting_cycle(
    section_title: str,
    section_search_results_str: str,
    raw_materials: str,
    query: str,
    max_reflections: int = 2,
) -> str:
    """起草 -> 反思 -> 修订 的 ReAct 风格循环。"""

    def build_prompt(feedback: Optional[str] = None) -> str:
        feedback_block = f"\n\n**改进建议:**\n{feedback}\n\n请基于以上建议进行修订，并保留可引用的 URL 标记。" if feedback else ""
        return f"""
        **章节主题:** {section_title}
    ...
    draft = await Runner.run(DraftingAgent, build_prompt())
    ...
    for attempt in range(max_reflections):
        ...
        如果满足，请只输出“满足”二字。如果不满足，列出需要补充或改进的要点，每行一个。
        """
        reflection = await Runner.run(ReflectionAgent, reflection_prompt)
        reflection_text = reflection.final_output.strip()

        if reflection_text == "满足":
            return draft.final_output

        print(f"[ReAct] {section_title} 反思建议:\n{reflection_text}")

        # ReAct: 反思 -> 行动（带着反馈再写一版）
        revision_prompt = build_prompt(feedback=reflection_text)
        draft = await Runner.run(DraftingAgent, revision_prompt)

    return draft.final_output


async def process_section(section: Dict[str, Any], query: str, index: int) -> str:
    """并发处理单个章节，包含检索、抓取和反思式起草。"""
    ...
    crawled_contents = await asyncio.gather(*crawl_tasks) if crawl_tasks else []
    ...
    try:
        section_body = await run_drafting_cycle(
            section_title=section_title,
            section_search_results_str=section_search_results_str,
            raw_materials=raw_materials,
            query=query,
        )
        print(f"-> 章节起草完成: {section_title}")
    except Exception as e:
        section_body = f"章节起草失败: {e}"
...
"""
    ...
    # 3. 并发执行章节生成（含反思修订）
    print("\nStep 3: 并发生成章节内容（带 ReAct 反思回路）...")
    section_tasks = [
        asyncio.create_task(process_section(section, query, i))
