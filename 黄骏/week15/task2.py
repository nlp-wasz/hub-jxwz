...
def generate_with_qwen(
    ...
    try:
        ...
        print(f"[warn] openai package not available: {exc}")
        return None
...
def ensure_mineru_output(pdf_path: Path, output_dir: Path) -> Path:
    """Run mineru if the parsed JSON does not exist; return content JSON path."""
    stem = pdf_path.stem
    content_path = output_dir / stem / "auto" / f"{stem}_content_list.json"
...
def main() -> None:
    ...
    parser.add_argument(
        "--content",
        default="mineru_output/small_ocr/auto/small_ocr_content_list.json",
