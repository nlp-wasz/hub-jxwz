...
# 读取数据
ratings = pd.read_csv("../03_推荐系统/M_ML-100K/ratings.dat", sep="::", header=None, engine='python')
ratings.columns = ['user_id', 'movie_id', 'rating', 'timestamp']

movies = pd.read_csv("../03_推荐系统/M_ML-100K/movies.dat", sep="::", header=None, engine='python', encoding='latin-1')
movies.columns = ['movie_id', 'movie_title', 'movie_tag']

PROMPT_TEMPLATE = """
你是一个电影推荐专家，请结合用户历史观看的电影，推荐用户未来可能观看的电影。
如下是历史观看的电影:
{0}
        ...
        if len(matched) > 0:
            ...
            for keyword in keywords:
                if len(keyword) > 3:  # 只使用长度大于3的关键词
                    matched = available_movies[
                        available_movies['movie_title'].str.contains(keyword, case=False, na=False, regex=False)
                    ]
                    if len(matched) > 0:
                        recommended_movies.append(matched.iloc[0])
                        break
