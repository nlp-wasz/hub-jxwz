...
mcp = FastMCP(
    ...
    instructions="""This server contains some fast_api of tools.""",
)
...
def get_address_detail(address_text: Annotated[str, "City Name"]):
    ...
    try:
        return requests.get(f"https://whyta.cn/api/tx/addressparse?key={TOKEN}&text={address_text}", timeout=5).json()[
            "result"]
    except:
        return []
...
def get_tel_info(tel_no: Annotated[str, "Tel phone number"]):
    ...
    try:
        return requests.get(f"https://whyta.cn/api/tx/mobilelocal?key={TOKEN}&phone={tel_no}", timeout=5).json()[
            "result"]
    except:
        return []
...
def get_scenic_info(scenic_name: Annotated[str, "Scenic/tourist place name"]):
    ...
    try:
        return \
        requests.get(f"https://whyta.cn/api/tx/scenic?key={TOKEN}&word={scenic_name}", timeout=5).json()["result"][
            "list"]
    except:
        return []
