...
def get_k_data(tool_name, tool_args):
    ...
    tool_args_startDate = datetime.strptime(tool_args["startDate"], "%Y-%m-%d").strftime("%Y-%m-%d")
    tool_args_endDate = datetime.strptime(tool_args["endDate"], "%Y-%m-%d").strftime("%Y-%m-%d")
    tool_args_type = tool_args.get("type", 0)
    ...
    with st.chat_message("assistant"):
        ...
        with st.spinner("正在请求LLM回答..."):
            ...
            with st.spinner("正在绘制K线图"):
                ...
                re_llm_res = re.search(r"```json\s*([\s\S]*?)\s*```", llm_res, re.I).group(1).strip()

                # 解析成功
                if len(re_llm_res) > 0:
                    # 获取方法名
                    tool_name = re_llm_res.split(":")[0]
                    # 如果方法名如下，则获取参数信息 以及 绘制K线图
                    if tool_name in ["get_stock_month_kline", "get_stock_week_kline", "get_stock_day_kline"]:
                        try:
                            tool_args = re_llm_res[re_llm_res.index(":") + 1:]
