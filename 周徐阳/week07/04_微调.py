...
"""
Qwen-LoRA实体识别微调
- 使用Qwen模型和LoRA适配器进行实体识别任务微调
"""

from transformers import (
    AutoTokenizer,  # Qwen分词器
    AutoModelForTokenClassification,  # Qwen模型用于token分类
    TrainingArguments,  # trainer参数
    Trainer,  # trainer
...
# 初始化tokenizer
model_name = "Qwen/Qwen2-7B"  # 或其他Qwen模型路径
...
peft_config = LoraConfig(
    ...
    lora_dropout=0.1,
    bias="none",
    target_modules=["q_proj", "k_proj", "v_proj"]  # 注意Qwen模型的模块名称可能不同
)
...
def compute_metrics(p):
    ...
    # 计算宏平均F1分数
    macro_f1 = np.mean([f1_scores[key] for key in f1_scores if key.endswith('_f1')])
...
# 保存模型
trainer.save_model()
tokenizer.save_pretrained('./ner-qwen-lora-model')
