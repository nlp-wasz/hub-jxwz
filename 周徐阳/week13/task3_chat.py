...
def build_chat_and_stock_agents(
        task: str,
        external_client: AsyncOpenAI,
        mcp_server: Optional[MCPServerSse],
        tool_use_behavior: str,
) -> tuple[Agent, Agent]:
    stock_prompt = prompt_with_handoff_instructions(
        f"{get_init_message('股票分析')}\n\n- 如果当前话题不是股票、证券或投资相关内容，请交还给 ChatAgent 处理。"
    )
    chat_prompt = prompt_with_handoff_instructions(
        f"{get_init_message(task)}\n\n- 当用户的需求涉及股票、证券、行情、财报、K线或投资策略时，请调用 transfer_to_stock_agent 将对话移交给股票代理。"
    )

    stock_agent = Agent(
        name="StockAgent",
        handoff_description="处理股票行情、K线查询、代码检索、财报和投资风险分析。",
        instructions=stock_prompt,
    ...
    chat_agent = Agent(
        name="ChatAgent",
        handoff_description="负责闲聊、通用问答以及非股票话题的交流。",
        instructions=chat_prompt,
        handoffs=[handoff(
            stock_agent,
            tool_description_override="当问题涉及股票、证券或投资分析时，交由股票代理处理。"
        )],
        mcp_servers=[mcp_server] if mcp_server else [],
        model=OpenAIChatCompletionsModel(
            model=os.environ["OPENAI_MODEL"],
    ...
    stock_agent.handoffs = [handoff(
        chat_agent,
        tool_description_override="当对话转向日常闲聊或非股票问题时，将对话交回 ChatAgent。"
    )]
...
async def chat(user_name:str, session_id: Optional[str], task: Optional[str], content: str, tools: List[str] = []):
    ...
    if tools and set(need_viz_tools) & set(tools):
    ...
    chat_agent, stock_agent = build_chat_and_stock_agents(
        task or "",
        external_client,
        mcp_server,
        tool_use_behavior,
    )
    starting_agent = stock_agent if task == "股票分析" else chat_agent

    assistant_message_parts: List[str] = []

    async def stream_result(result):
        async for event in result.stream_events():
            if event.type == "raw_response_event" and isinstance(event.data, ResponseOutputItemDoneEvent):
                if isinstance(event.data.item, ResponseFunctionToolCall):
                    ...
                    yield payload
                    assistant_message_parts.append(payload)

            if event.type == "raw_response_event" and isinstance(event.data, ResponseTextDeltaEvent):
                if event.data.delta:
                    yield event.data.delta
                    assistant_message_parts.append(event.data.delta)

    async def run_agent(agent_to_run: Agent):
        result = Runner.run_streamed(agent_to_run, input=content, session=session)
        async for chunk in stream_result(result):
            yield chunk

    async with (mcp_server if mcp_server else _noop_async_cm()):
        async for chunk in run_agent(starting_agent):
            yield chunk

    append_message2db(session_id, "assistant", "".join(assistant_message_parts))
