...
async def process_section(
    ...
    """并发生成单个章节，包含搜索、抓取、起草与反馈改写。"""
    section_title = section.get("section_title") or f"章节 {index + 1}"
    search_keywords = section.get("search_keywords") or query
    ...
    async with semaphore:
        ...
        try:
            ...
            urls_to_crawl = [res["url"] for res in search_results if res.get("url")][:max_urls]
        except Exception:
        ...
        crawled_pages = await asyncio.gather(*crawl_tasks) if crawl_tasks else []
        crawled_content = []
        for url, content in zip(urls_to_crawl, crawled_pages):
        ...
        **原始网页内容 (请基于此内容撰写):**
        ...
        请按照 ReAct 风格先列出关键线索，再给出最终的 Markdown 章节。保持专注于章节主题。
        """

        feedback = ""
        draft_output = ""
        for attempt in range(1, max_revision_rounds + 1):
            prompt = base_draft_prompt
            if feedback:
                prompt += f"\n请根据以下反馈改写并补充: \n{feedback}\n仅输出改写后的章节内容。"
            ...
            draft_output = section_draft.final_output
            ...
            请判断该章节是否满足用户提问和章节主题的要求。
            如果满足，请仅回复“满足”。
            如果不满足，请列出需要进一步分析或补充的子问题，每行一个。
            """
            reflection = await Runner.run(
                ReflectionAgent,
                reflection_prompt,
            )
            feedback_candidate = reflection.final_output.strip()

            if feedback_candidate.startswith("满足"):
                print(f"-> 章节审核通过 (第 {attempt} 轮): {section_title}")
                break

            feedback = feedback_candidate
            print(f"-> 章节需要改进 (第 {attempt} 轮): {section_title} | 反馈: {feedback_candidate}")
        else:
            print(f"-> 达到反馈轮数上限，保留最新稿件: {section_title}")
...
async def deep_research(
    ...
    """
    执行深度研究流程：规划 -> 并发检索 -> 抓取 -> 起草 + ReAct 反馈 -> 整合。
    """
...
"""
    ...
    # 3. 并发逐章进行检索、抓取和起草 + ReAct 反馈
    max_concurrent_sections = max(1, max_concurrent_sections or len(sections))
    max_revision_rounds = max(1, max_revision_rounds)
    print(f"并发章节数: {max_concurrent_sections}，每章最多反馈轮次: {max_revision_rounds}")
