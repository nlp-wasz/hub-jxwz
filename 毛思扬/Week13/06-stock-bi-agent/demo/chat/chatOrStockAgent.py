...
if "messages" not in st.session_state.keys() or "session_id" in st.session_state.keys():
    st.session_state.messages = [
        {"role": "system", "content": "你好，我是闲聊agent和股票agent。"}
    ]
    if "session_id" in st.session_state.keys() and st.session_state.session_id:
        data = requests.post(
            "http://127.0.0.1:8000/v1/chatOrStockAgent/get?session_id=" + st.session_state['session_id']).json()
...
async def request_chat(content: str, user_name: str, session_id: str) -> str:
    url = "http://127.0.0.1:8000/v1/chatOrStockAgent/"
...
def request_session_id() -> str:
    url = "http://127.0.0.1:8000/v1/chatOrStockAgent/init"
    headers = {
        "Content-Type": "application/json"
    }
        ...
        with st.chat_message("assistant"):  # 大模型输出
            ...
            with st.spinner("请求中..."):
                async def stream_output():
                    accumulated_text = ""
                    response_generator = request_chat(prompt, st.session_state['user_name'],
