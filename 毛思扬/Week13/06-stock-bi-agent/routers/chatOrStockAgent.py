...
import services.chatOrStockAgent as chatOrStockAgent_services
from models.data_models import BasicResponse, RequestForChat

router = APIRouter(prefix="/v1/chatOrStockAgent", tags=["chatOrStockAgent"])
...
async def chat(req: RequestForChat) -> StreamingResponse:
    try:
        ...
        async def chat_stream_generator():
            # async for 遍历异步数据流
            async for chunk in chatOrStockAgent_services.chat(
...
async def init_chat() -> StreamingResponse:
    try:
        return BasicResponse(
            code=200, message="ok",
            data={
                "session_id": chatOrStockAgent_services.generate_random_chat_id()
            }
        )
    except Exception as e:
        print(traceback.format_exc())
...
def get_chat(session_id: str) -> BasicResponse:
    try:
        response = chatOrStockAgent_services.get_chat_sessions(session_id)
        return BasicResponse(
            code=200, message="ok",
            data=response
        )
...
def delete_chat(session_id: str) -> BasicResponse:
    try:
        response = chatOrStockAgent_services.delete_chat_session(session_id)
...
def list_chat(user_name: str) -> BasicResponse:
    try:
        chat_records = chatOrStockAgent_services.list_chat(user_name)
...
def feedback_chat(session_id: str, message_id: int, feedback: bool) -> BasicResponse:
    try:
        chatOrStockAgent_services.change_message_feedback(session_id, message_id, feedback)
        return BasicResponse(code=200, message="ok", data=[])
    except Exception as e:
        print(traceback.format_exc())
        return BasicResponse(code=500, message=traceback.format_exc(), data=[])
