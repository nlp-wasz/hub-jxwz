...
os.environ["OPENAI_API_KEY"] = "sk-dfa65577922849b1aeb1cb4f366bc4dc"
...
# --- 修改代理定义 ---
# 3.3. 反思代理 (Reflection Agent) - 新增
reflection_system_prompt = """
你是一名专业的内容评审专家。你的任务是对一份报告章节初稿的质量进行评估，并提出改进建议。

你需要严格按照以下步骤进行评估：
1. **分析目标**: 明确该章节的主题和应覆盖的核心要点。
2. **审查内容**: 检查初稿是否完整、准确地涵盖了这些要点。
3. **识别不足**: 找出缺失的重要信息、逻辑不严谨之处或表述不清的地方。
4. **提出建议**: 给出具建设性的反馈意见，指导如何改进。

你的输出必须是纯文本格式，包含两个部分，用 "--- Feedback ---" 分隔开：
第一部分是简短的总体评价（一句话概括）；
第二部分是详细的反馈建议列表，每条建议占一行，以 "- " 开头。

示例输出格式：
章节整体较为全面，但在某些细节上还需加强。
--- Feedback ---
- 建议补充更多关于XXX的具体案例说明
- 第二段中的YY部分论述不够深入，建议扩展
- ZZ术语未作解释，读者可能难以理解
"""
...
"""
    ...
    # 3. 并发处理所有章节
    print("\nStep 3: 并发处理所有章节...")
    tasks = [process_section_with_refinement(section, i + 1) for i, section in enumerate(sections)]
...
# --- 新增函数：带反思机制的章节处理 ---
async def process_section_with_refinement(section: Dict[str, Any], index: int) -> str:
    """
    带有反思和优化机制的章节处理流程。
    """
    section_title = section.get("section_title")
    search_keywords = section.get("search_keywords")
    print(f"\n--- 处理章节 [{index}]: {section_title} ---")
    ...
    while iteration_count < MAX_REFINEMENT_ITERATIONS:
        iteration_count += 1
        print(f"  -> 章节 [{index}] 第 {iteration_count} 轮迭代")
        ...
        if iteration_count == 1:
            ...
            **搜索结果摘要:**
            {section_search_results_str[:3000]}... (供参考)
            
            **原始网页内容:**
            {raw_materials}

            请根据以上反馈建议，修订 {section_title} 章节的内容。
            """
        ...
        if iteration_count < MAX_REFINEMENT_ITERATIONS:
            ...
            **章节草稿:**
            ...
            try:
                ...
                feedback_parts = reflection_response.final_output.split("--- Feedback ---")
                overall_comment = feedback_parts[0].strip()
                feedback = feedback_parts[1].strip() if len(feedback_parts) > 1 else ""

                print(f"    总体评价: {overall_comment}")
                print(f"    反馈建议:\n{feedback}")

                # 如果没有收到具体的反馈建议，则提前终止迭代
            ...
            print(f"    达到最大迭代次数，结束优化")

    print(f"-> 章节 [{index}] 最终完成: {section_title}")
    return f"## {section_title}\n\n{current_draft}"
