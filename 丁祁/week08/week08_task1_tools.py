...
class ClassBaseInput(BaseModel):
    """
        根据用户提供的信息进行 意图识别 + 领域识别 + 实体识别
    """
    intent: str = Field(description="用户的意图")

    intent: Literal["OPEN", "SEARCH", "REPLAY_ALL", "NUMBER_QUERY", "DIAL",  "CLOSEPRICE_QUERY", "SEND", "LAUNCH", "PLAY", "REPLY","RISERATE_QUERY", "DOWNLOAD", "QUERY", "LOOK_BACK", "CREATE", "FORWARD", "DATE_QUERY", "SENDCONTACTS", "DEFAULT", "TRANSLATION", "VIEW", "NaN", "ROUTE", "POSITION"] = Field(description="意图标签")
    domain: Literal["music", "app", "radio", "lottery", "stock", "novel", "weather", "match", "map", "website", "news", "message", "contacts", "translation", "tvchannel", "cinemas", "cookbook", "joke", "riddle", "telephone", "video", "train", "poetry", "flight", "epg", "health", "email", "bus", "story"] = Field(description="领域标签")
    # slots: List[Literal[
    ...
    # ]] = Field(description="实体标签")
    slots: List[str] = Field(description="实体标签")


def class_base_on_input_tools(query_str):
    print("query_str", query_str)

    result = ExtractionAgent(model_name = "qwen-plus").call(query_str, ClassBaseInput)
    print(result)
    return result

# class_base_on_input_tools("帮我查询下从北京到天津到武汉的汽车票")
