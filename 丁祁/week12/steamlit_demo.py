...
with st.sidebar:
    ...
    tool_name = st.selectbox("选择工具", ["", "news", "tools"])
...
async def get_model_response(prompt, model_name, use_tool, tool_name):
    def tool_filter_func(tool, server_context):
        """根据 tool_name 过滤单个工具，返回 True 表示保留，False 表示过滤掉"""
        # 正确获取工具名称 - 通过 tool.tool 访问实际的工具对象
        if tool_name == "news":
            return {"allowed_tool_names": NEWS_TOOLS}
        elif tool_name == "tools":
            return {"allowed_tool_names": GENERAL_TOOLS}
        return None
...
if len(key) > 1:
    if prompt := st.chat_input():
        ...
        with st.chat_message("assistant"):
            ...
            with st.spinner("请求中..."):
                try:
                    response_generator = get_model_response(prompt, model_name, use_tool, tool_name)
